...
def favarite():
    ...
    if data is None:
      ...
      print(favarite_info)
      print(favarite_info[0][2])   #tweet_id
      ...
      html = render_template('favorites.html', user=userinfo,favarites=list)
      #DBの切断
      db.close()
      con.close()
      print(data)
...
def plofile(username=None):
    ...
    if data is None:    #cookieにuser情報がないときの処理 ログイン画面に飛ばす
      ...
      html = render_template('profile.html', user=user,tweets=tweet_list)
    return html
