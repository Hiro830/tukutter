...
def kansuu(x):  #ってか関数使えた!!リストは返すことができる!!
####ここでuser + tweetInfo + favarite + followをくっつけて全部で使えれば超便利!!
    db = MySQLdb.connect( user='root', passwd='A12qwerzxcv123', host='localhost', db='tukutter_2', charset='utf8')
    con = db.cursor()
    #####ここでsqlのtable結合しまくって必要な情報をまとめる######
    ###なかなかに難しい、、、
...
#cookieのuser情報を取得
    ...
    print (userinfo)
#userがフォローしてる人のidを取得
    sql = 'SELECT follow_you FROM follow WHERE follow_me = %s group by follow_you'
    con.execute(sql,[userinfo[0]])
    result = con.fetchall()
    follow_id = result
    print(follow_id[1][0])

#フォローしてる人のツイートとuser情報を合わせて取得
    for row in follow_id:
        sql = 'SELECT * from tweetInfo inner join users on tweetInfo.users_id = users.id WHERE users_id = %s'
        con.execute(sql,[row[0]])
        tubuyakis = con.fetchall()
        for row in tubuyakis: #ツイート情報を個別に配列する
            list.append(row)
    print(list)
    print('------------------------')
    print(list[0])
    print(list[1])
#取得したツイートがお気に入りかどうか判定する
    #お気に入りがある場合は2字配列で返ってくる
    sql = 'SELECT * from favorite WHERE tweet_id = %s and user_id = %s'
    con.execute(sql,[list[1][0],userinfo[0]])
    ...
    db.close()  #DBの切断
    con.close()
#ツイート＋user情報をばらしてお気に入りを追加するようにしたい
    return 'get'
...
def follow(username=None):
    ...
    if data is None:
      ...
      if follow is ():   #値がないときって()でいいのか？？
        html = redirect('http://' + host + '/top')  ####local環境とsever上の環境とは起動が違う,,,####
...
def index3():
    ...
    if data is None:    #cookieにuser情報がないときの処理 ログイン画面に飛ばす
      ...
      db.close()
      con.close()
      print (kansuu(data))  #関数にcookieにあるuser情報を渡して情報を取得!!!
    return html
...
def tubuyaki1():
    ...
    return render_template('tweet.html',name=data)
